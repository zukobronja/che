...
public class MavenProjectGenerator implements CreateProjectHandler {
  ...
  public void onCreateProject(
    ...
    if (options == null || options.isEmpty() || !options.containsKey(GENERATOR_OPTION)) {
      strategies
          .get(SIMPLE_GENERATION_STRATEGY)
          .generateProject(projectWsPath, attributes, options);
    } else {
      if (strategies.containsKey(options.get(GENERATOR_OPTION))) {
        strategies
            .get(options.get(GENERATOR_OPTION))
            .generateProject(projectWsPath, attributes, options);
      } else {
